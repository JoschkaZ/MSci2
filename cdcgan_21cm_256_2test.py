...
class CGAN():
    ...
    def build_discriminator(self, img, con):
        ...
        hid = Conv2D(128, kernel_size=5, strides=2, padding="same")(img)
        hid = BatchNormalization(momentum=0.8)(hid)
        hid = LeakyReLU(alpha=0.2)(hid)
        # -> 128x128x128

        hid = Conv2D(194, kernel_size=5, strides=2, padding="same")(hid)
        hid = BatchNormalization(momentum=0.8)(hid)
        hid = LeakyReLU(alpha=0.2)(hid)
        # -> 194x64x64

        hid = Conv2D(294, kernel_size=5, strides=2, padding="same")(hid)
        hid = BatchNormalization(momentum=0.8)(hid)
        hid = LeakyReLU(alpha=0.2)(hid)
        # -> 294x32x32

        hid = Conv2D(446, kernel_size=5, strides=2, padding="same")(hid)
        hid = BatchNormalization(momentum=0.8)(hid)
        hid = LeakyReLU(alpha=0.2)(hid)
        # -> 446x16x16

        hid = Conv2D(676, kernel_size=5, strides=2, padding="same")(hid)
        hid = BatchNormalization(momentum=0.8)(hid)
        hid = LeakyReLU(alpha=0.2)(hid)
        # -> 676x8x8

        hid = Conv2D(1204, kernel_size=5, strides=2, padding="same")(hid)
        hid = BatchNormalization(momentum=0.8)(hid)
        hid = LeakyReLU(alpha=0.2)(hid)
        # -> 1204x4x4
