...
class CGAN():
    ...
    def build_generator(self, noise, con):
        ...
        for i in range(twopot-1):
            ...
            #hid = Conv2D(int(np.round((cto/cfrom)**((i+1)/(twopot-1))*cfrom)), kernel_size=5, padding="same")(hid)
            hid = Conv2DTranspose(int(np.round((cto/cfrom)**((i+1)/(twopot-1))*cfrom)), 5, strides=2, padding='same')(hid)
            hid = BatchNormalization(momentum=0.9)(hid)
            hid = ReLU()(hid)

            #hid = Conv2D(int(np.round((cto/cfrom)**((i+1)/(twopot-1))*cfrom)), kernel_size=5, strides=1, padding='same')(hid)
            #hid = BatchNormalization(momentum=0.9)(hid)
            #hid = ReLU()(hid)
    ...
    def build_discriminator(self, img, con):
        ...
        # TODO
        # try removing noise and swapping
        # try putting back in slow down for discriminator
        # try having additional conv layers

        hid = Flatten()(hid)
        #hid = Dropout(0.1)(hid) # NOTE LARGE DROPOUT
        ...
        for epoch in range(epochs):
            ...
            if epoch% 100 == 0:
                ...
                for j in range(len(imgs)):
                    print(labels[j])
                    print(np.max(imgs[j]), np.min(imgs[j]), np.mean(imgs[j]), np.std(imgs[j]))
                    print(np.max(gen_imgs[j]), np.min(gen_imgs[j]), np.mean(gen_imgs[j]), np.std(gen_imgs[j]))
...
if __name__ == '__main__':
    ...
    if args[1] == 'new':
        ...
        cgan.train(epochs=4000000, batch_size=8, sample_interval=100, save_model_interval = 1000, stats_interval = 10)
